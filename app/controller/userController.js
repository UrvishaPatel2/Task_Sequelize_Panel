...
exports.login = async (req, res, next) => {
    try {
        ...
        if (!user) {
            ...
            if (validPassword) {
                const JwtToken = jwt.sign({ email: email }, process.env.JWT_SECRET, { expiresIn: '15m' });
                next(new GeneralResponse(
                    "Login Done..",
                    { token: JwtToken },
                    config.HTTP_CREATED
                ))
...
exports.updateProfile = async (req, res, next) => {
    try {
        ...
        const updateData = await User.update(data, { where: { email: req.user.email } });
        if (updateData) {
            next(new GeneralResponse(
                "Data Updated",
                updateData,
    ...
    try {
        ...
        if (user) {
            ...
            if (validPassword) {
                ...
                const updatePassword = await User.update({ password: bcryptPassword }, { where: { email: req.user.email } });
                if (updatePassword) {
                    next(new GeneralResponse(
