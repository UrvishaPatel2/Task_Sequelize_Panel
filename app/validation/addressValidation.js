...
module.exports = {
    addressValid: Joi.array().items(Joi.object({
        ...
        country: Joi.string().empty().required().valid('india', 'maldives', 'congo', 'germany').messages({
        ...
        state: Joi.string().empty().required().valid('gujarat', 'goa', 'punjab', 'rajasthan').messages({
            "string.base": `State should be a type of text`,
            "string.empty": 'State is not allowed to be empty',
            "string.required": `State is Required`,
        }),
        city: Joi.string().empty().required().valid('navsari', 'ahmedabad', 'surat', 'gandhinagar').messages({
            "string.base": `City should be a type of text`,
            "string.empty": 'City is not allowed to be empty',
            "string.required": `City is Required`,
        }),
        pincode: Joi.string().pattern(/^[0-9]+$/).length(6).empty().required().label("Pincode").messages({
